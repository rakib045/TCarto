...
square_grid = 64
input_data_file_area = 'input/donut_apple_calorie_weight_64_64.txt'
output_img_filename = 'output_DivCon_donut_apple'
input_img_file = "input/donut_apple.jpg"

input_data_file_saturation = 'input/donut_apple_water_weight_64_64.txt'
input_data_file_light = 'input/donut_apple_fat_weight_64_64.txt'
...
if __name__ == "__main__":
    ...
    min_sat_range = 1
    max_sat_range = 100
    min_sat_val = np.min(values_actual_sat)
    max_sat_val = np.max(values_actual_sat)

    min_light_range = 1
    max_light_range = 100
    min_light_val = np.min(values_actual_light)
    max_light_val = np.max(values_actual_light)

    x_unit = input_image.size[0] / grid_count_horizontal_actual
    y_unit = input_image.size[1] / grid_count_vertical_actual

    for i in range(input_image.size[0]):  # for every col:
        for j in range(input_image.size[1]):  # For every row

            pix_val_sat = min_sat_range + (values_actual_sat[m.floor(j/x_unit)][m.floor(i/y_unit)] - min_sat_val) / (max_sat_val - min_sat_val) * (max_sat_range - min_sat_range)
            pix_val_light = min_light_range + (values_actual_light[m.floor(j / x_unit)][m.floor(i / y_unit)] - min_light_val) / (max_light_val - min_light_val) * (max_light_range - min_light_range)
            '''
            pixels[i, j] = (pixels[i, j][0],
                            int(pix_val_sat * pixels[i, j][1]),
                            int(pix_val_light))
            '''
            pixels[i, j] = (pixels[i, j][0],
                            int(pixels[i, j][1] + pix_val_sat)
                            , int(pixels[i, j][2] - pix_val_light))


    input_image.show()
    input_image = input_image.convert("RGBA")
    input_image.save('output/' + output_img_filename + '_after_SV.png', 'PNG')
